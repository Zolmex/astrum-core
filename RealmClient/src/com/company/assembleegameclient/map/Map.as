...
public class Map extends Sprite
{
   ...
   public function update(time:int, dt:int) : void
   {
      ...
      for each(go in this.goDict_)
      {
         ...
         else
         {
            ...
            {
               if (!go.isInvulnerable() && go.isTargetable())
               {
   ...
   public function draw(camera:Camera, time:int) : void
   {
      ...
      // draw breath overlay
      if(this.player_ != null && this.player_.oxygen_ >= 0 && this.player_.oxygen_ < Parameters.BREATH_THRESH)
      {
         b = (Parameters.BREATH_THRESH - this.player_.oxygen_) / Parameters.BREATH_THRESH;
